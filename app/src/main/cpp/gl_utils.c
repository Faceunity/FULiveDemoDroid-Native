...
GLuint loadShader(GLenum type, const char *source) {
    ...
    if (compiled == GL_FALSE) {
        LOGE("Could not compile shader 0x%d", compiled);
...
void createProgram() {
    ...
    GLuint fragmentShader = loadShader(GL_FRAGMENT_SHADER, FRAGMENT_SHADER_2D);
    ...
    if (linkStatus != GL_TRUE) {
        LOGE("Could not link program: 0x%d", linkStatus);
